...
class Line():
    ...
    def check_fit(self, fit_params, curva):
        #new_curva = np.abs(curva)
        #old_curva = np.mean(self.curvas)
...
class Lane():
    ...
    def adjust_fit(self, left_fit, ret_l, right_fit, ret_r):
        ...
        '''
        count_l = len(self.left.allx)
        count_r = len(self.right.allx)
        
        #left line has more points detected than right
        if (count_l - count_r) > count_l //4:
            self.right.set_fit(left_fit, 300)
        #right line has more points detected than left
        elif (count_r - count_l) > count_r //4:
            self.left.set_fit(right_fit, -300)
        # right is straighter than left
        elif np.abs(self.right.radius) > np.abs(self.left.radius):
            self.left.set_fit(right_fit, -300)
        # left is straighter than right
        else:
            self.right.set_fit(left_fit, 300)
